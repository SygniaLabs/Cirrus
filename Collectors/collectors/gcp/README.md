...
2. **Configurations:** current GCP configuration data (resource hierarchy map, role bindings, and service account info)
   via the Cloud Asset Inventory service
...
Specification** |
|-------------------------------------------------------------------------------------------------------------|--------------------------|
| Mapping of resource hierarchy <br/>*(only available when targeting folder(s) or the organization
resource)* | gcp_map |
...
gather log previews against multiple resources at the same time.
Example [log preview output](./example_log_preview_output.png).
...
1. Add the service account identity to an ingress rule allowing access to targeted project(s) and service(s) **(
   preferred)**
