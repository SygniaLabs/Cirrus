...
class Parser:
    def __init__(self):
        # Main parser
        self.parser = ArgumentParser('mirage.py gcp',
    ...
    def validate_log_collection_args(parser, args):
        if args.logs is None and args.custom_logs is None and not args.preview:
            parser.error(
                'specify at least one action: [--logs LOG1,LOG2..] or [--custom-logs CL1,CL2..] or [--preview]')
        ...
        if (args.project_id or args.folder_id or args.organization_id) and not args.preview and \
                (args.start_time is None or args.end_time is None):
            parser.error(
                'specify start and end timestamps: [--start-time YYYY-MM-DDTHH:MM:SSZ] [--end-time YYYY-MM-DDTHH:MM:SSZ]')
    ...
    def validate_config_collection_args(parser, args):
        ...
        if args.organization_id and (args.project_id or args.folder_id):
            parser.error(
                'specify one resource tier at a time: [--project-id ID1,ID2.. | --folder-id ID1,ID2.. | --organization-id ID]')
    ...
    @staticmethod
    def _get_resources_from_args(parser, args):
        # Resource tier parse
        resource_ids = []
        if args.project_id:
            project_ids = list(args.project_id.split(','))
            for pid in project_ids:
                resource_ids.append(f'projects/{pid}')
        if args.folder_id:
            folder_ids = list(args.folder_id.split(','))
            for fid in folder_ids:
                resource_ids.append(f'folders/{fid}')
        if args.organization_id:
            if ',' in args.organization_id:
                parser.error('specify only a single Organization ID')
            resource_ids.append(f'organizations/{args.organization_id}')
        return resource_ids

    @staticmethod
    def _get_creds_from_args(parser, args):
    ...
    def user_cli_preview_logs(parser, args):
        ...
        credential_file = Parser._get_creds_from_args(parser, args)
        # Resource tier parse
        resource_ids = Parser._get_resources_from_args(parser, args)
    ...
    def user_cli_log_collection(parser, args):
        ...
        if args.module == 'logs':
            log_selection = list(args.logs.split(','))
        ...
        # Custom log parse
        if args.custom_logs:
            custom_selection = list(args.custom_logs.split(','))
        # Timestamp parse
        start_time = args.start_time
        end_time = args.end_time
        data = {'credential_file': credential_file,
                'resource_ids': resource_ids,
                'start_time': start_time,
                'end_time': end_time}
        if args.logs:
            data['log_selection'] = log_selection
        if args.custom_logs:
    ...
    def user_cli_config_collection(parser, args):
        ...
        config_selection = list(args.configs.split(','))
